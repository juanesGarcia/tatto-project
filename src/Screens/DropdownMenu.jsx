...
if(newText.length <= 323){
  ...
  try {
    const response = await axios.put(`https://backed-tatto.onrender.com/editar/${post_id}`, {
  ...
  const handleEliminar = async () => {
    try {
      const response = await axios.delete(`https://backed-tatto.onrender.com/deleteimages/${post_id}`);
      console.log(response);
      Swal.fire({
        icon: 'success',
        title: response.data.message,
