          ...
          <div className="reactions-list">
            <div className='reactionsitems'>
              {visibleReactions.map((user) => (
                <div className='namereactions' key={user.id}><Avatar sx={{ width: 50, height: 50 }} src={user.media_url}>{user.name[0]}</Avatar><div className='name'  onClick={() => handlemove(user.id, user.name)}>{user.name}</div></div>
              ))}
            </div>
          </div>
